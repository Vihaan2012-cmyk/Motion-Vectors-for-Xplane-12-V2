...
-- ---- THE LOG BOX EARNS THE SPACE, SO IT GETS IT.
--
-- It was 132px, which showed roughly seven lines - and the integrity check
-- alone writes more than that, so the interesting line ("Vulkan layer has not
-- reported yet", a failed check) scrolled off before it could be read. The
-- window grows by exactly the same amount the box does, so nothing below it
-- moves relative to the box and the bottom row stays where it was.
local LOG_H = 232
local WIN_W, WIN_H = 720, 360 + (LOG_H - 132)
...
local function build(w, x, y)
    ...
    imgui.BeginChild("log", -1, LOG_H, true)
    ...
    imgui.TextUnformatted("")
    -- ---- TWO KEYS, BECAUSE THEY GENUINELY DIFFER IN WHEN THEY APPLY.
    --
    -- fsr.replace decides whether our compute shader is substituted for
    -- X-Plane's own FSR module. Substitution happens at module creation, before
    -- a frame is ever drawn, so the layer latches it once per process - no
    -- amount of writing it mid-flight can change this run.
    --
    -- fsr.backend_fsr3 gates the per-frame DISPATCH, and the layer now checks
    -- it on every one. Clearing it stands FSR3 down on the next frame and
    -- leaves X-Plane's own upscale on screen, which is a real off switch and
    -- not a promise about the next launch.
    --
    -- So FSR is ON only when both are set, and the button writes both. Turning
    -- it off always applies at once. Turning it back on applies at once too IF
    ...
    if fsr_on then
        text(C_AMBER, "FSR selected. It drives X-Plane's own FSR switch, so that")
        text(C_AMBER, "must be ON in X-Plane's graphics settings as well.")
        text(C_DIM, "Click FSR again to switch it off. That applies immediately.")
    else
        -- Deliberately not claiming which of the two cases this run is in. The
        -- panel reads the file, and the file cannot say whether THIS process
        -- latched the substitution at startup - so a confident "no restart
        -- needed" would be a guess that is wrong half the time.
