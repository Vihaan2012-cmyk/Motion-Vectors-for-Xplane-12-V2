...
--[[ ===========================================================================
  ...
  (%TEMP%\rcp_live.ini), which the layer re-reads every few frames. So the panel
...
local function try_dataref(name, kind)
    ...
    local var = name:gsub("/", "_"):gsub("^taaimpl_", "RCP_"):gsub("^sim_", "SIM_")
    local ok = pcall(dataref, var, name, kind)
    have[name] = ok
    return ok
end

try_dataref("crashphys/layer_attached",     "readonly")
try_dataref("crashphys/render_scale",       "readonly")
try_dataref("crashphys/viewport_w",         "readonly")
try_dataref("crashphys/viewport_h",         "readonly")
try_dataref("crashphys/jitter_phases",      "readonly")
try_dataref("crashphys/moving_objects",     "readonly")
try_dataref("crashphys/reverse_z",          "readonly")
try_dataref("crashphys/lod_bias",           "writable")
try_dataref("crashphys/vram_used_mb",       "readonly")
try_dataref("crashphys/vram_budget_mb",     "readonly")
try_dataref("crashphys/vram_total_mb",      "readonly")
try_dataref("crashphys/velocity_mb",        "readonly")
try_dataref("crashphys/pipelines_patched",  "readonly")
try_dataref("crashphys/pipelines_rejected", "readonly")
...
local function ini_path()
    -- RCP_LIVE_FILE FIRST, exactly as the layer resolves it.
    --
...
local function verify_integrity()
    ...
    -- exists.
    if get("crashphys/layer_attached", 0) == 1 then
        logf(C_GREEN, "  OK      Vulkan layer is attached to this process")
    elseif not have["crashphys/layer_attached"] then
...
local function first_frame()
    started = true
    logf(C_AMBER, "Realistic Crash Physics  -  version %s", MOD_VERSION)
    verify_integrity()
...
local function build(w, x, y)
    ...
    if attach_pending and get("crashphys/layer_attached", 0) == 1 then
    ...
    if get("crashphys/layer_attached", 0) ~= 1 then
        ...
        heading("INJECTION")
        local rejected = get("crashphys/pipelines_rejected", 0)
        row("PIPELINES CARRYING VELOCITY",
            string.format("%d", get("crashphys/pipelines_patched", 0)))
        row("REJECTED BY THE DRIVER", string.format("%d", rejected),
            rejected == 0 and C_GREEN or C_RED)

        heading("RENDER")
        row("RESOLUTION", string.format("%d x %d", get("crashphys/viewport_w", 0),
                                                  get("crashphys/viewport_h", 0)))
        row("SCALE", string.format("%.2f x", get("crashphys/render_scale", 1.0)))
        row("DEPTH", get("crashphys/reverse_z", 0) == 1 and "reverse-Z" or "standard")
        row("JITTER", string.format("%d phases", get("crashphys/jitter_phases", 0)))
        row("MOVING OBJECTS", string.format("%d", get("crashphys/moving_objects", 0)))

        heading("MEMORY")
        row("PROCESS VRAM", string.format("%d MB", get("crashphys/vram_used_mb", 0)))
        row("DRIVER BUDGET", string.format("%d / %d MB",
            get("crashphys/vram_budget_mb", 0), get("crashphys/vram_total_mb", 0)), C_DIM)

        heading("TUNING")
        local changed, v = imgui.SliderFloat("LOD bias", get("crashphys/lod_bias", 0.0),
                                             -2.0, 0.0, "%.2f")
        if changed and have["crashphys/lod_bias"] then RCP_lod_bias = v end
    end
...
add_macro("Realistic Crash Physics: panel",
...
create_command("FlyWithLua/MotionVectors/toggle_panel",
               "Realistic Crash Physics: toggle panel",
               "if mv_open then mv_open() end", "", "")
